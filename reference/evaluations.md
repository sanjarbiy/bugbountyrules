...
- Computes the break-even (`context window × bytes per token`) and **does not extract the 60 KB
...
is keyed on `(endpoint × parameter × class)`, but a killed false positive is not a cell — it is an
