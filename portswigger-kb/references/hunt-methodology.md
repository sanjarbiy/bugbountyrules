...
### 6. PROVE
- Reproduce 3x across fresh sessions. Demonstrate cross-account / real-data impact with two accounts.
...
Every surface x every applicable class = checked or proven inapplicable (with reason).
```
...
| "Found one bug, done." | Hunt siblings (cluster rule). Then chain A->B->C. Chains pay 3-10x. |
...
```
✓ Reproducible (3x, fresh sessions)
