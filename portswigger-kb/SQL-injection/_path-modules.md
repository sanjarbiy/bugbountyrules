...
**Coverage check:** all path modules (What is SQLi → How to detect → in-different-parts → retrieving hidden data → subverting logic → UNION ×4 → examining the DB → blind → conditional responses → error-based → time delays → OAST → different contexts → second-order → prevention) read 0→end and each mapped to a distilled sub-technique folder above. All 18 labs covered in those folders' `## Labs` sections.
