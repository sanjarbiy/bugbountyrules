...
5. Intruder (single thread), payload on the char: `SUBSTRING(password,§1§,1)='§a§'`, positionxcharset. The ~10000ms row = each char. Reassemble, log in -> solved.
...
- **Burp Intruder:** single-threaded resource pool, watch "Response received" ms; cluster-bomb positionxchar.
