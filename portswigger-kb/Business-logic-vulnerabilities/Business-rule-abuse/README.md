...
**Infinite money:**
- Any gift card + discount coupon combo → calculate: if `purchase_price × (1 - discount) < face_value` → profitable loop.
...
4. Send `GET /my-account` to Intruder → Null payloads × 412 → max concurrent=1 → run.
5. Result: ~412 × $3 = $1236 credit → buy jacket.
...
- **Burp Intruder** — null payloads × 412 for infinite money attack
...
$10 gift card + 30% coupon = $7 cost → $10 redemption → net +$3. Burp macro automates the buy+redeem cycle. Intruder: null payloads × 412, concurrency=1. After attack: store credit ≈ $1236 → buy jacket. Key insight: gift card arbitrage + macro automation = unlimited credit.
