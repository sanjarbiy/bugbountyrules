...
POST /cart  productId=2  quantity=-100  -> reduces cart total by 100× price
...
4. Stop, clear cart. Recalculate: leather jacket = 133700 cents. Need total ∈ [0, 10000].
   - 323 × 99 + 47 additional jackets → total = −$1221.96 (under $100 credit after a filler item).
...
Intruder: POST /cart  productId=1&quantity=99  Null payloads × 323, concurrency=1
...
**Lab 2 (int overflow):** Intruder (null payloads × 323, qty=99) → total wraps to negative → add 47 → pad with cheap item → checkout.
...
Qty maxes at 2-digit per request. Intruder: null payloads × 323, qty=99, concurrency=1. Wait for total to wrap (32-bit int overflow → negative). Then add 47 jackets + cheap padding item to land total in [0, $100]. Key insight: 32-bit signed integer wraps at 2,147,483,647 → -2,147,483,648.
