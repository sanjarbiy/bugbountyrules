...
Maintain a LIVE ledger, updated as you go - **any durable form works**: an `engagement/coverage.md`, a **class x surface x result table** (often the cleanest - one row per vuln-class, columns for surface tested and outcome), or a persistent memory / knowledge-base file (findings themselves go in Rule 17's `interesting_findings.json` or equivalent). The form is yours; the discipline is not. Critically, the ledger MUST carry an **"open leads" section** - genuine trails not yet closed (async links, protocols needing RE, tool-blocked confirmations). An open lead means you are NOT done (Rule 25), even with nothing to submit. For every `(endpoint x parameter x vuln-class)` cell, record exactly one state:
...
**Stop ONLY when the ledger (Rule 21) shows every reachable in-scope surface x class TESTED-CLEAN / VULNERABLE / SKIPPED, every class climbed to exhaustion (Rule 22), every lead chased.** Then:
...
2. Your **coverage ledger** (Rule 21) proves EVERY reachable in-scope surface x class is TESTED-CLEAN-with-evidence / VULNERABLE / SKIPPED-with-reason - **and** you have already expanded the surface (new subdomains, hidden endpoints, APK/JS re-mined) at least once more.
